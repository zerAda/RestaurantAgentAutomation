...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
  WHERE conname = 'chk_tenants_plan'
) THEN
ALTER TABLE tenants
...
CREATE INDEX IF NOT EXISTS idx_api_clients_active_hash ON api_clients (is_active, token_hash);
-- ----- restaurant_users -----
CREATE TABLE IF NOT EXISTS restaurant_users (
  ...
  role text NOT NULL CHECK (
    role IN ('customer', 'owner', 'admin', 'kitchen')
  ),
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  delivery_fee_cents int NULL CHECK (
    delivery_fee_cents IS NULL
    OR delivery_fee_cents >= 0
  ),
  delivery_eta_min int NULL CHECK (
    delivery_eta_min IS NULL
    OR delivery_eta_min >= 0
  ),
  delivery_eta_max int NULL CHECK (
    delivery_eta_max IS NULL
    OR delivery_eta_max >= 0
  ),
  delivery_slot_id uuid NULL,
  delivery_slot_start timestamptz NULL,
  delivery_slot_end timestamptz NULL,
  -- Payment (COD / no-show)
  payment_mode text DEFAULT 'COD' CHECK (
    payment_mode IN (
      'COD',
      'DEPOSIT_COD',
      'CIB',
...
CREATE TABLE IF NOT EXISTS outbound_messages (
  ...
  user_id text NOT NULL,
  order_id uuid NULL REFERENCES orders(order_id) ON DELETE
  SET NULL,
    template text NOT NULL DEFAULT 'reply',
    payload_json jsonb NOT NULL DEFAULT '{}'::jsonb,
    status text NOT NULL DEFAULT 'PENDING' CHECK (
      status IN ('PENDING', 'RETRY', 'SENT', 'DLQ', 'DROPPED')
    ),
...
CREATE INDEX IF NOT EXISTS idx_outbound_due ON outbound_messages (status, next_retry_at);
CREATE INDEX IF NOT EXISTS idx_outbound_rest_channel ON outbound_messages (restaurant_id, channel, created_at DESC);
-- ----- inbound_messages -----
...
CREATE INDEX IF NOT EXISTS idx_inbound_messages_window ON inbound_messages(conversation_key, received_at DESC);
-- ----- idempotency_keys -----
...
CREATE TABLE IF NOT EXISTS conversation_quarantine (
  ...
  release_policy text NOT NULL DEFAULT 'AUTO_RELEASE' CHECK (
    release_policy IN ('AUTO_RELEASE', 'CONFIRMATION', 'MANUAL')
  ),
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_message_templates_key_locale_tenant ON message_templates (template_key, locale, tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_order_status_history_order_time ON order_status_history(order_id, created_at ASC);
-- ----- delivery_zones -----
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_delivery_zones_rest_wilaya_commune ON delivery_zones (restaurant_id, lower(wilaya), lower(commune));
CREATE INDEX IF NOT EXISTS idx_delivery_zones_active ON delivery_zones (restaurant_id, is_active);
-- ----- delivery_fee_rules -----
CREATE TABLE IF NOT EXISTS delivery_fee_rules (
  ...
  free_delivery_threshold_cents int NULL CHECK (
    free_delivery_threshold_cents IS NULL
    OR free_delivery_threshold_cents >= 0
  ),
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_delivery_fee_rules_active ON delivery_fee_rules (restaurant_id, is_active);
-- ----- delivery_time_slots -----
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_delivery_slots_def ON delivery_time_slots (restaurant_id, day_of_week, start_time, end_time);
CREATE INDEX IF NOT EXISTS idx_delivery_slots_active ON delivery_time_slots (restaurant_id, day_of_week, is_active);
...
CREATE INDEX IF NOT EXISTS idx_slot_reservations_slot ON delivery_slot_reservations (slot_id, reserved_at DESC);
...
ALTER TABLE orders
ADD CONSTRAINT fk_orders_delivery_slot FOREIGN KEY (delivery_slot_id) REFERENCES delivery_time_slots(slot_id) ON DELETE
SET NULL;
END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS address_clarification_requests (
  ...
  status text NOT NULL DEFAULT 'OPEN' CHECK (
    status IN ('OPEN', 'RESOLVED', 'HANDOFF', 'CANCELLED')
  ),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_address_clarify_status ON address_clarification_requests (status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_address_clarify_conversation ON address_clarification_requests (conversation_key)
...
CREATE TABLE IF NOT EXISTS support_tickets (
  ...
  reason_code text NOT NULL DEFAULT 'HELP' CHECK (
    reason_code IN (
      'HELP',
      'DELIVERY_AMBIGUOUS',
      'PAYMENT_ISSUE',
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_support_ticket_active_conversation ON support_tickets(restaurant_id, conversation_key)
WHERE status IN ('OPEN', 'ASSIGNED');
CREATE INDEX IF NOT EXISTS idx_support_tickets_status_rest ON support_tickets(restaurant_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_support_tickets_customer ON support_tickets(
  restaurant_id,
  channel,
  customer_user_id,
  created_at DESC
...
CREATE INDEX IF NOT EXISTS idx_support_ticket_messages_ticket ON support_ticket_messages(ticket_id, created_at DESC);
-- ----- support_assignments -----
...
CREATE INDEX IF NOT EXISTS idx_support_assignments_admin ON support_assignments(admin_user_id, assigned_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_faq_entries_active_rest_locale ON faq_entries(restaurant_id, locale)
WHERE is_active;
...
CREATE INDEX IF NOT EXISTS idx_fraud_rules_scope_active ON fraud_rules (scope, is_active);
-- ----- payment_intents -----
...
CREATE INDEX IF NOT EXISTS idx_payment_intents_conversation ON payment_intents(conversation_key);
...
CREATE INDEX IF NOT EXISTS idx_payment_history_intent ON payment_history(payment_intent_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS customer_payment_profiles (
  ...
  trust_score integer NOT NULL DEFAULT 50 CHECK (
...
CREATE INDEX IF NOT EXISTS idx_customer_payment_profiles_user ON customer_payment_profiles(user_id);
CREATE INDEX IF NOT EXISTS idx_customer_payment_profiles_blacklist ON customer_payment_profiles(soft_blacklisted)
...
CREATE TABLE IF NOT EXISTS restaurant_payment_config (
  ...
  deposit_percentage integer NOT NULL DEFAULT 30 CHECK (
...
ADD CONSTRAINT chk_customer_preferences_locale CHECK (lower(locale) IN ('fr', 'ar', 'darija'));
END $$;
CREATE INDEX IF NOT EXISTS idx_customer_preferences_tenant_locale ON customer_preferences(tenant_id, locale);
-- ----- token_usage_log -----
CREATE TABLE IF NOT EXISTS token_usage_log (
  id bigserial PRIMARY KEY,
  client_id uuid REFERENCES api_clients(client_id) ON DELETE
  SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_token_usage_log_client_time ON token_usage_log(client_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_token_usage_log_hash_time ON token_usage_log(token_hash, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_replay_guard_message_id ON webhook_replay_guard(message_id)
...
ALTER TABLE admin_wa_audit_log
ADD CONSTRAINT chk_admin_wa_audit_action CHECK (
    action IN (
      'help',
      'tickets',
      'take',
...
END $$;
CREATE INDEX IF NOT EXISTS idx_admin_wa_audit_tenant_time ON admin_wa_audit_log(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_admin_wa_audit_actor_time ON admin_wa_audit_log(actor_phone, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_admin_wa_audit_action_time ON admin_wa_audit_log(action, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_admin_wa_audit_target ON admin_wa_audit_log(target_type, target_id)
...
CREATE INDEX IF NOT EXISTS idx_admin_phone_allowlist_phone ON admin_phone_allowlist(phone_number)
WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_admin_phone_allowlist_tenant ON admin_phone_allowlist(tenant_id, restaurant_id)
WHERE is_active = true;
-- ----- system_flags -----
...
CREATE TABLE IF NOT EXISTS structured_logs (
  ...
  channel text CHECK (
    channel IS NULL
    OR channel IN ('whatsapp', 'instagram', 'messenger', 'tiktok')
  ),
  workflow_name text,
  node_name text,
...
CREATE INDEX IF NOT EXISTS idx_structured_logs_correlation_id ON structured_logs(correlation_id);
CREATE INDEX IF NOT EXISTS idx_structured_logs_created_at ON structured_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_structured_logs_level_created ON structured_logs(level, created_at DESC)
WHERE level IN ('WARN', 'ERROR');
CREATE INDEX IF NOT EXISTS idx_structured_logs_tenant_created ON structured_logs(tenant_id, created_at DESC)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_darija_patterns_unique ON darija_patterns(category, lower(pattern));
-- ----- daily_metrics -----
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_commune_ref_wilaya_name ON commune_reference(wilaya_code, lower(name_fr));
-- ----- orders_audit -----
...
CREATE INDEX IF NOT EXISTS idx_retention_runs_started_at ON ops.retention_runs (run_started_at DESC);
-- ----- ops_kv -----
...
CREATE TABLE IF NOT EXISTS admin_audit_log (
  id bigserial PRIMARY KEY,
  tenant_id uuid NULL REFERENCES tenants(tenant_id) ON DELETE
  SET NULL,
    restaurant_id uuid NULL REFERENCES restaurants(restaurant_id) ON DELETE
  SET NULL,
    actor_client_id uuid NULL REFERENCES api_clients(client_id) ON DELETE
  SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_admin_audit_tenant_time ON admin_audit_log (tenant_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_drivers_status ON drivers(restaurant_id, status);
...
CREATE INDEX IF NOT EXISTS idx_assignments_driver_status ON delivery_assignments(driver_id, status);
-- ----- marketing_trigger_log -----
CREATE TABLE IF NOT EXISTS marketing_trigger_log (
...
CREATE INDEX IF NOT EXISTS idx_marketing_log_exec ON marketing_trigger_log(executed_at DESC);
-- ===========================
-- GOD TIER SCHEMA ADDITIONS
-- ===========================
-- ----- THE NERVOUS SYSTEM (Event Bus) -----
CREATE TABLE IF NOT EXISTS system_event_bus (
  event_id bigserial PRIMARY KEY,
  event_type text NOT NULL,
  -- e.g. 'ORDER_CREATED', 'WEATHER_ALERT', 'STOCK_LOW'
...
CREATE INDEX IF NOT EXISTS idx_event_bus_status_prio ON system_event_bus(status, priority DESC, created_at ASC);
-- ----- KITCHEN SYMPHONY (Stations & Steps) -----
CREATE TABLE IF NOT EXISTS kitchen_stations (
  station_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(tenant_id) ON DELETE CASCADE,
  restaurant_id uuid NOT NULL REFERENCES restaurants(restaurant_id) ON DELETE CASCADE,
  name text NOT NULL,
  -- 'Grill', 'Fryer', 'Cold Station', 'Assembly'
  capabilities text [] DEFAULT '{}',
  -- ['burger', 'meat']
  printer_ip text,
  is_active boolean NOT NULL DEFAULT true
);
CREATE TABLE IF NOT EXISTS menu_item_prep_steps (
  step_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  menu_item_code text NOT NULL,
  -- Linked to menu_items(item_code) manually
  station_id uuid NOT NULL REFERENCES kitchen_stations(station_id) ON DELETE CASCADE,
  step_name text NOT NULL,
  -- 'Griller le steak'
  duration_seconds int NOT NULL DEFAULT 60,
  step_order int NOT NULL DEFAULT 1,
  start_offset_sec int NOT NULL DEFAULT 0
);
CREATE INDEX IF NOT EXISTS idx_prep_steps_item ON menu_item_prep_steps(menu_item_code, step_order);
-- ===========================
-- SECTION 4: Functions
-- ===========================
...
v_surcharge int := 0;
v_free int := NULL;
BEGIN IF p_restaurant_id IS NULL THEN RETURN QUERY
SELECT false,
  false,
  0,
  0,
...
LIMIT 1;
IF z.zone_id IS NULL THEN RETURN QUERY
SELECT false,
  false,
  0,
  0,
...
RETURN;
END IF;
IF NOT z.is_active THEN RETURN QUERY
SELECT true,
  false,
  z.fee_base_cents,
  0,
...
RETURN;
END IF;
IF COALESCE(p_total_cents, 0) < COALESCE(z.min_order_cents, 0) THEN RETURN QUERY
SELECT true,
  true,
  z.fee_base_cents,
  0,
...
t_local := (
  p_at AT TIME ZONE COALESCE(
    (
      ...
      WHERE rest.restaurant_id = p_restaurant_id
    ),
    'Africa/Algiers'
  )
)::time;
...
ORDER BY dfr.surcharge_cents DESC LOOP IF r.start_time <= r.end_time THEN IF t_local >= r.start_time
  AND t_local < r.end_time THEN v_surcharge := COALESCE(r.surcharge_cents, 0);
v_free := r.free_delivery_threshold_cents;
EXIT;
END IF;
ELSE IF t_local >= r.start_time
OR t_local < r.end_time THEN v_surcharge := COALESCE(r.surcharge_cents, 0);
v_free := r.free_delivery_threshold_cents;
EXIT;
END IF;
END IF;
...
AND COALESCE(p_total_cents, 0) >= v_free THEN RETURN QUERY
SELECT true,
  true,
  z.fee_base_cents,
  v_surcharge,
...
CREATE OR REPLACE FUNCTION public.create_order(p_conversation_key text) RETURNS TABLE(
    order_id uuid,
    total_cents int,
    summary text,
...
SELECT cs.tenant_id,
  ...
  COALESCE(cs.state_json, '{}'::jsonb) AS state_json INTO v_tenant,
  v_restaurant,
  v_channel,
  v_user,
  v_stage,
...
WHERE cs.conversation_key = p_conversation_key FOR
UPDATE;
IF v_restaurant IS NULL THEN RAISE EXCEPTION 'Unknown conversation_key %',
p_conversation_key;
END IF;
IF v_stage = 'PLACED'
AND v_last_order <> '' THEN RETURN QUERY
SELECT v_last_order::uuid,
  COALESCE(
    (
      SELECT o.total_cents
      FROM public.orders o
      WHERE o.order_id = v_last_order::uuid
    ),
    0
  ),
  (
  ...
  (
    SELECT o.delivery_fee_cents
    FROM public.orders o
    WHERE o.order_id = v_last_order::uuid
  ),
  (
    SELECT COALESCE(o.total_cents, 0) + COALESCE(o.delivery_fee_cents, 0)
    FROM public.orders o
    WHERE o.order_id = v_last_order::uuid
  ),
  (
    SELECT o.delivery_eta_min
    FROM public.orders o
    WHERE o.order_id = v_last_order::uuid
  ),
  (
    SELECT o.delivery_eta_max
    FROM public.orders o
    WHERE o.order_id = v_last_order::uuid
  );
RETURN;
...
SELECT COALESCE(
    (
      SELECT c.cart_json->>'serviceMode'
      FROM public.carts c
      WHERE c.conversation_key = p_conversation_key
    ),
    (
      SELECT cs2.state_json->>'serviceMode'
      FROM public.conversation_state cs2
      WHERE cs2.conversation_key = p_conversation_key
    ),
    'a_emporter'
  ) INTO v_mode;
...
IF v_mode = 'livraison' THEN IF v_wilaya IS NULL
OR v_commune IS NULL THEN RAISE EXCEPTION 'DELIVERY_ADDRESS_MISSING';
END IF;
END IF;
...
lines AS (
  SELECT (elem->>'item')::text AS item_code,
    GREATEST(1, LEAST(20, COALESCE((elem->>'qty')::int, 1))) AS qty,
    COALESCE(elem->'options', '[]'::jsonb) AS options_json
  FROM cart,
    LATERAL jsonb_array_elements(COALESCE(cart.cart_json->'items', '[]'::jsonb)) elem
),
priced AS (
  SELECT l.item_code,
    l.qty,
    ...
    COALESCE(
      (
        ...
        FROM jsonb_array_elements_text(l.options_json) oc(option_code)
          JOIN public.menu_item_options mo ON mo.restaurant_id = v_restaurant
          AND mo.option_code = oc.option_code
      ),
      0
    ) AS opt_cents
  FROM lines l
    JOIN public.menu_items mi ON mi.restaurant_id = v_restaurant
...
IF q.reason <> 'OK' THEN RAISE EXCEPTION '%',
q.reason;
END IF;
...
SET state_json = jsonb_set(
    jsonb_set(
      v_state,
      '{stage}',
      to_jsonb('PLACED'::text),
      true
    ),
    '{last_order_id}',
    to_jsonb(v_order::text),
    true
  ),
  updated_at = now()
...
SELECT v_order,
  ...
  (
    SELECT v_items_total + COALESCE(o.delivery_fee_cents, 0)
    FROM public.orders o
    WHERE o.order_id = v_order
  ),
  (
...
CREATE OR REPLACE FUNCTION public.normalize_locale(p_locale text) RETURNS text LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE loc text := lower(trim(coalesce(p_locale, '')));
BEGIN IF loc IN (
  'darija',
  'dz',
  'darja',
...
END IF;
IF loc LIKE 'ar%' THEN RETURN 'ar';
END IF;
IF loc IN ('ar', 'ar-dz', 'ar_dz', 'arabic') THEN RETURN 'ar';
END IF;
IF loc IN ('fr', 'fr-fr', 'fr_fr', 'french') THEN RETURN 'fr';
END IF;
RETURN 'fr';
END $$;
-- ----- detect_darija -----
CREATE OR REPLACE FUNCTION public.detect_darija(p_text text) RETURNS boolean LANGUAGE plpgsql STABLE AS $$
DECLARE txt text := lower(trim(coalesce(p_text, '')));
match_count int := 0;
BEGIN
...
DECLARE loc text := public.normalize_locale(p_locale);
eta_txt text := '';
link_txt text := '';
...
BEGIN IF p_eta_min IS NOT NULL
OR p_eta_max IS NOT NULL THEN eta_txt := E'\nETA: ' || COALESCE(p_eta_min::text, '') || CASE
  WHEN p_eta_max IS NOT NULL THEN '-' || p_eta_max::text
  ELSE ''
END || ' min';
END IF;
IF p_status_link IS NOT NULL
AND length(trim(p_status_link)) > 0 THEN link_txt := E'\nSuivi: ' || trim(p_status_link);
END IF;
...
WHERE tenant_id = '_GLOBAL'
  ...
  AND locale = loc
LIMIT 1;
IF tmpl IS NOT NULL THEN out_txt := replace(
  replace(tmpl, '{{order_id}}', order_short),
  '{{eta}}',
  eta_txt
);
...
SELECT order_id,
  ...
  user_id,
  delivery_eta_min,
  delivery_eta_max INTO o
FROM public.orders
WHERE order_id = p_order_id;
IF o.order_id IS NULL THEN RETURN '{}'::jsonb;
END IF;
...
txt := public.wa_order_status_text(
  loc,
  p_customer_status,
  p_order_id,
  o.delivery_eta_min,
...
v_dedupe text;
v_payload jsonb;
BEGIN IF p_customer_status IS NULL THEN RETURN;
END IF;
SELECT tenant_id,
  restaurant_id,
  channel,
...
FROM public.orders
WHERE order_id = p_order_id;
IF o.tenant_id IS NULL THEN RETURN;
END IF;
IF lower(COALESCE(o.channel, '')) <> 'whatsapp' THEN RETURN;
END IF;
v_dedupe := 'order_status:' || p_order_id::text || ':' || upper(p_customer_status);
v_payload := public.build_wa_order_status_payload(
  p_order_id,
  upper(p_customer_status),
  p_status_link,
  p_locale
);
...
VALUES (
    ...
    o.user_id,
    p_order_id,
    'WA_ORDER_STATUS_' || upper(p_customer_status),
    v_payload,
    'PENDING',
    now()
...
CREATE OR REPLACE FUNCTION public.compute_cart_total(p_conversation_key text) RETURNS int LANGUAGE plpgsql AS $$
...
IF v_restaurant IS NULL THEN RETURN 0;
END IF;
...
SELECT COALESCE(SUM((base_cents + opt_cents) * qty), 0)::int INTO v_total
FROM priced;
...
SELECT tenant_id,
  restaurant_id,
  channel,
  user_id INTO v_tenant,
  v_restaurant,
  v_channel,
  v_user
FROM public.conversation_state
WHERE conversation_key = p_conversation_key;
INSERT INTO public.conversation_quarantine(
    conversation_key,
    reason,
    active,
    expires_at,
...
CREATE OR REPLACE FUNCTION public.release_expired_quarantines(p_limit int DEFAULT 50) RETURNS TABLE(
    ...
    reason text,
    expires_at timestamptz
  ) LANGUAGE plpgsql AS $$ BEGIN RETURN QUERY WITH due AS (
    SELECT q.id
...
END $$;
-- ----- fraud_eval_checkout -----
CREATE OR REPLACE FUNCTION public.fraud_eval_checkout(p_conversation_key text) RETURNS TABLE(
    action text,
    score int,
    total_cents int,
    reason text,
...
v_quar int := 30;
v_cancel_cnt int := 0;
BEGIN v_total := public.compute_cart_total(p_conversation_key);
SELECT cs.user_id INTO v_user
FROM public.conversation_state cs
WHERE cs.conversation_key = p_conversation_key;
SELECT COALESCE((params_json->>'threshold_cents')::int, v_thr),
  COALESCE(
    (params_json->>'confirm_ttl_minutes')::int,
    v_ttl
  ) INTO v_thr,
  v_ttl
FROM public.fraud_rules
WHERE scope = 'CHECKOUT'
  AND rule_key = 'CO_HIGH_ORDER_TOTAL'
  AND is_active = true
LIMIT 1;
SELECT COALESCE(
    (params_json->>'cancel_limit')::int,
    v_cancel_limit
  ),
  COALESCE(
    (params_json->>'window_days')::int,
    v_window_days
  ),
  COALESCE(
    (params_json->>'quarantine_minutes')::int,
    v_quar
  ) INTO v_cancel_limit,
  v_window_days,
  v_quar
FROM public.fraud_rules
WHERE scope = 'CHECKOUT'
  AND rule_key = 'CO_REPEAT_CANCELLED_7D'
  AND is_active = true
LIMIT 1;
...
END IF;
IF v_cancel_cnt >= v_cancel_limit THEN RETURN QUERY
SELECT 'QUARANTINE',
  90,
  v_total,
  'REPEAT_CANCELLED',
...
RETURN;
END IF;
IF v_total >= v_thr THEN RETURN QUERY
...
VALUES (p_message_hash, p_message_id, p_channel) ON CONFLICT (message_hash, channel) DO NOTHING;
RETURN FOUND;
END $$;
-- ----- cleanup_replay_guard -----
CREATE OR REPLACE FUNCTION public.cleanup_replay_guard(p_max_age_hours int DEFAULT 24) RETURNS int LANGUAGE plpgsql AS $$
DECLARE v_deleted int;
BEGIN
DELETE FROM public.webhook_replay_guard
WHERE received_at < now() - make_interval(hours => p_max_age_hours);
...
CREATE OR REPLACE FUNCTION public.record_daily_metrics(
    p_key text,
    p_value bigint DEFAULT 1,
    p_channel text DEFAULT NULL
  ) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
INSERT INTO public.daily_metrics(metric_date, metric_key, metric_value, channel)
VALUES (CURRENT_DATE, p_key, p_value, p_channel) ON CONFLICT (metric_date, metric_key, channel) DO
UPDATE
SET metric_value = daily_metrics.metric_value + p_value,
  updated_at = now();
END $$;
-- ----- record_latency_sample -----
CREATE OR REPLACE FUNCTION public.record_latency_sample(
    p_workflow text,
    p_latency_ms int,
    p_channel text DEFAULT NULL
  ) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
...
DECLARE v_id bigint;
BEGIN
INSERT INTO public.admin_wa_audit_log(
    tenant_id,
    restaurant_id,
    actor_phone,
    actor_role,
...
END $$;
-- ----- check_admin_allowed -----
CREATE OR REPLACE FUNCTION public.check_admin_allowed(p_phone text) RETURNS TABLE(
    allowed boolean,
    role text,
    display_name text,
    permissions jsonb
...
CREATE OR REPLACE FUNCTION public.get_system_flag(p_key text, p_default text DEFAULT NULL) RETURNS text LANGUAGE plpgsql STABLE AS $$
DECLARE v text;
BEGIN
SELECT flag_value INTO v
FROM public.system_flags
WHERE flag_key = p_key;
RETURN COALESCE(v, p_default);
END $$;
CREATE OR REPLACE FUNCTION public.set_system_flag(p_key text, p_value text, p_by text DEFAULT NULL) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
INSERT INTO public.system_flags(flag_key, flag_value, updated_at, updated_by)
VALUES (p_key, p_value, now(), p_by) ON CONFLICT (flag_key) DO
UPDATE
...
END $$;
-- ----- fn_orders_audit -----
CREATE OR REPLACE FUNCTION public.fn_orders_audit() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN IF TG_OP = 'INSERT' THEN
INSERT INTO public.orders_audit (
    order_id,
    action,
    new_status,
    new_total_cents,
...
RETURN NEW;
ELSIF TG_OP = 'UPDATE' THEN IF OLD.status IS DISTINCT
FROM NEW.status
  OR OLD.total_cents IS DISTINCT
FROM NEW.total_cents THEN
INSERT INTO public.orders_audit (
    order_id,
    action,
    old_status,
...
CREATE OR REPLACE FUNCTION public.fn_api_clients_audit() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN IF TG_OP = 'INSERT' THEN
INSERT INTO public.api_clients_audit (
    client_id,
    action,
    new_is_active,
    new_scopes,
...
RETURN NEW;
ELSIF TG_OP = 'UPDATE' THEN IF OLD.is_active IS DISTINCT
FROM NEW.is_active
  OR OLD.scopes IS DISTINCT
FROM NEW.scopes THEN
INSERT INTO public.api_clients_audit (
    client_id,
    action,
    old_is_active,
...
CREATE OR REPLACE FUNCTION public.faq_entries_tsv_update() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN NEW.search_tsv := setweight(
...
CREATE OR REPLACE FUNCTION public.purge_old_inbound_messages(p_days INT DEFAULT 90) RETURNS INT LANGUAGE plpgsql AS $$
DECLARE v_deleted INT;
BEGIN
...
CREATE OR REPLACE FUNCTION public.trg_orders_init_tracking() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.last_notified_status IS NULL THEN NEW.last_notified_status := NULL;
END IF;
IF NEW.last_notified_at IS NULL THEN NEW.last_notified_at := NULL;
END IF;
RETURN NEW;
END $$;
...
END IF;
IF NEW.last_notified_status IS NOT NULL
AND NEW.last_notified_status = v_customer THEN RETURN NEW;
END IF;
IF NEW.last_notified_at IS NOT NULL
AND NEW.last_notified_at > now() - v_window THEN v_next := NEW.last_notified_at + v_window;
ELSE v_next := now();
END IF;
...
WHERE order_id = NEW.order_id
  AND dedupe_key = (
    'order_status:' || NEW.order_id::text || ':' || upper(v_customer)
  );
RETURN NEW;
END $$;
-- ===========================
...
CREATE TRIGGER orders_init_tracking BEFORE
INSERT ON public.orders FOR EACH ROW EXECUTE FUNCTION public.trg_orders_init_tracking();
DROP TRIGGER IF EXISTS orders_status_tracking ON public.orders;
CREATE TRIGGER orders_status_tracking BEFORE
UPDATE OF status ON public.orders FOR EACH ROW EXECUTE FUNCTION public.trg_orders_status_tracking();
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_trigger
  WHERE tgname = 'trg_orders_audit'
) THEN CREATE TRIGGER trg_orders_audit
AFTER
INSERT
  OR
UPDATE
  OR DELETE ON public.orders FOR EACH ROW EXECUTE FUNCTION public.fn_orders_audit();
END IF;
END $$;
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_trigger
  WHERE tgname = 'trg_api_clients_audit'
) THEN CREATE TRIGGER trg_api_clients_audit
AFTER
INSERT
  OR
UPDATE
  OR DELETE ON public.api_clients FOR EACH ROW EXECUTE FUNCTION public.fn_api_clients_audit();
END IF;
END $$;
DROP TRIGGER IF EXISTS trg_faq_entries_tsv ON public.faq_entries;
CREATE TRIGGER trg_faq_entries_tsv BEFORE
INSERT
  OR
UPDATE OF question,
  answer,
  tags ON public.faq_entries FOR EACH ROW EXECUTE FUNCTION public.faq_entries_tsv_update();
-- ===========================
-- SECTION 6: Views
-- ===========================
...
SELECT t.tenant_id,
  ...
  (
    ...
    WHERE ac.tenant_id = t.tenant_id
      AND ac.is_active
  ) AS active_api_clients
FROM public.tenants t;
CREATE OR REPLACE VIEW public.v_request_trace AS
SELECT sl.correlation_id,
  sl.level,
  sl.event_type,
  sl.message,
  sl.workflow_name,
  sl.node_name,
  sl.created_at AS log_ts,
  im.msg_id,
  im.channel,
  im.received_at AS inbound_ts,
  om.template,
...
CREATE INDEX IF NOT EXISTS idx_outbound_messages_status_retry ON outbound_messages(status, next_retry_at)
...
-- Fraud rules
INSERT INTO fraud_rules(rule_key, scope, action, score, params_json)
VALUES (
    'IN_FLOOD_30S',
    'INBOUND',
    'QUARANTINE',
    80,
    '{"limit_30s":6,"quarantine_minutes":10}'::jsonb
  ),
  (
    'IN_LONG_TEXT',
    'INBOUND',
  ...
  (
    ...
    'REQUIRE_CONFIRMATION',
    70,
    '{"threshold_cents":30000,"confirm_ttl_minutes":10}'::jsonb
  ),
  (
    'CO_REPEAT_CANCELLED_7D',
    'CHECKOUT',
    'QUARANTINE',
    90,
    '{"cancel_limit":3,"window_days":7,"quarantine_minutes":30}'::jsonb
  ) ON CONFLICT (rule_key) DO NOTHING;
-- Security event types
INSERT INTO ops.security_event_types(code, description)
VALUES ('SPAM_DETECTED', 'Inbound spam/flood detected'),
  ('BOT_SUSPECTED', 'Inbound bot/payload suspected'),
  ('QUARANTINE_APPLIED', 'Conversation quarantined'),
  (
    'QUARANTINE_RELEASED',
    'Conversation quarantine released'
  ),
  (
    'FRAUD_CONFIRMATION_REQUIRED',
  ...
  (
    'DELIVERY_MIN_ORDER',
    'Delivery: minimum order not reached'
  ),
  ('DELIVERY_QUOTE_OK', 'Delivery: quote computed'),
  (
    'ADDRESS_AMBIGUOUS',
    'Delivery: address clarification requested'
  ),
  (
    'LEGACY_TOKEN_ATTEMPT',
    'Legacy token auth attempt'
  ),
  ...
  ('RETENTION_RUN', 'Data retention purge executed') ON CONFLICT (code) DO NOTHING;
-- Darija detection patterns
INSERT INTO darija_patterns(category, pattern, priority)
VALUES ('menu', 'chno kayn', 10),
  ('menu', 'chnou kayen', 10),
  ('menu', 'wach kayn', 10),
  ('menu', 'wesh kayn', 10),
  ('menu', 'fin menu', 10),
  ...
  ('negative', 'la', 10),
  ('negative', 'lala', 10),
  ('negative', 'makanch', 10) ON CONFLICT (category, lower(pattern)) DO NOTHING;
...
VALUES (
    ...
    'CORE_CLARIFY',
    'fr',
    'Je n''ai pas bien compris. Tu peux preciser ?',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'CORE_CLARIFY',
    'ar',
    'لم أفهم جيداً. هل يمكنك التوضيح؟',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'CORE_CLARIFY',
    'darija',
    'Ma fhemtekch mezyan. 3awdha lik?',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
  ...
  (
    ...
    'CORE_MENU_HEADER',
    'ar',
    'القائمة (استخدم المعرفات في رسالتك)\n',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'CORE_LANG_SET_FR',
    'fr',
    'Langue definie sur Francais. Tape "menu" pour voir la carte.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'CORE_LANG_SET_AR',
    'ar',
    'تم تغيير اللغة إلى العربية. اكتب "menu" لعرض القائمة.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    ...
    'fr',
    'Commande confirmee (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    ...
    'fr',
    'Votre commande est en preparation (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    ...
    'fr',
    'Votre commande est prete (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    ...
    'fr',
    'Votre commande est en cours de livraison (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    ...
    'fr',
    'Commande livree / terminee (#{{order_id}}). Merci !',
    '["order_id"]'::jsonb
  ),
  (
    '_GLOBAL',
  ...
  (
    ...
    'WA_ORDER_STATUS_OUT_FOR_DELIVERY',
    'ar',
    'طلبك في الطريق للتوصيل (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    'WA_ORDER_STATUS_DELIVERED',
    'ar',
    'تم تسليم/إنهاء الطلب (#{{order_id}}). شكراً لك!',
    '["order_id"]'::jsonb
  ),
  (
    '_GLOBAL',
  ...
  (
    ...
    'WA_ORDER_STATUS_CONFIRMED',
    'darija',
    'Commande dyalek tconfirmat (#{{order_id}}).{{eta}}',
    '["order_id","eta"]'::jsonb
  ),
  (
    '_GLOBAL',
    'WA_ORDER_STATUS_DELIVERED',
    'darija',
    'Commande wslet / salat (#{{order_id}}). Choukran!',
    '["order_id"]'::jsonb
  ),
  (
    '_GLOBAL',
  ...
  (
    ...
    'SUPPORT_HANDOFF_ACK',
    'fr',
    'Merci. Un agent va vous contacter rapidement.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'SUPPORT_HANDOFF_ACK',
    'ar',
    'شكراً. سيتواصل معك أحد الموظفين قريباً.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'FAQ_NO_MATCH',
    'fr',
    'Je n''ai pas trouve de reponse. Je te mets en relation avec un agent.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'FAQ_NO_MATCH',
    'ar',
    'لم أجد إجابة. سأحوّلك إلى موظف.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'FRAUD_CONFIRM_REQUIRED',
    'fr',
    'Montant eleve detecte. Pour confirmer, reponds : CONFIRM {{code}} (valable {{minutes}} min).',
    '["code","minutes"]'::jsonb
  ),
  (
    '_GLOBAL',
    'FRAUD_CONFIRM_REQUIRED',
    'ar',
    'تم اكتشاف مبلغ مرتفع. للتأكيد أرسل: CONFIRM {{code}} (صالحة لمدة {{minutes}} دقيقة).',
    '["code","minutes"]'::jsonb
  ),
  (
    '_GLOBAL',
    'FRAUD_QUARANTINED',
    'fr',
    'Activite suspecte detectee. Ton acces est temporairement limite.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'FRAUD_QUARANTINED',
    'ar',
    'تم رصد نشاط مشبوه. تم تقييد الوصول مؤقتًا.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
    'FRAUD_RELEASED',
    'fr',
    'Acces retabli. Tu peux reprendre.',
    '[]'::jsonb
  ),
  (
    '_GLOBAL',
...
ANALYZE menu_items;
-- ----- GOD TIER INDEXES (High Performance) -----
-- Drivers: Speed up fleet lookups
CREATE INDEX IF NOT EXISTS idx_drivers_status_active ON drivers(status, is_active);
CREATE INDEX IF NOT EXISTS idx_drivers_location ON drivers(current_lat, current_lng);
-- Orders: Speed up "The Usual" history analysis
CREATE INDEX IF NOT EXISTS idx_orders_user_history ON orders(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_order_items_label ON order_items(label);
-- Event Bus: Speed up deduping and processing
CREATE INDEX IF NOT EXISTS idx_event_bus_type_date ON system_event_bus(event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_event_bus_status ON system_event_bus(status)
WHERE status = 'PENDING';
-- Kitchen: Speed up KDS filtering
CREATE INDEX IF NOT EXISTS idx_kitchen_station ON menu_item_prep_steps(station_id);
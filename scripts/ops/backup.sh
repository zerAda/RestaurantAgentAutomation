...
if docker compose exec -T postgres pg_dump -U n8n -d n8n --no-owner --no-acl -Fc > "$BACKUP_DIR/${BACKUP_NAME}-n8n.dump"; then
...
if docker compose exec -T postgres pg_dump -U n8n -d strapi --no-owner --no-acl -Fc > "$BACKUP_DIR/${BACKUP_NAME}-strapi.dump" 2>/dev/null; then

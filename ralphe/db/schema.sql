...
-- =========================
CREATE OR REPLACE FUNCTION create_order(p_conversation_key text) RETURNS TABLE(order_id uuid, total_cents int, summary text) LANGUAGE plpgsql AS $$
...
END IF;
SELECT COALESCE(
    cart_json->>'serviceMode',
    (
      SELECT state_json->>'serviceMode'
      FROM conversation_state
      WHERE conversation_key = p_conversation_key
    ),
    'a_emporter'
...
lines AS (
  ...
  FROM cart,
    LATERAL jsonb_array_elements(COALESCE(cart_json->'items', '[]'::jsonb)) elem
),
priced AS (
  SELECT l.item_code,
    l.qty,
    ...
    COALESCE(
      (
        ...
        FROM jsonb_array_elements_text(l.options_json) oc(option_code)
          JOIN menu_item_options mo ON mo.restaurant_id = v_restaurant
          AND mo.option_code = oc.option_code
      ),
      0
    ) AS opt_cents
  FROM lines l
    JOIN menu_items mi ON mi.restaurant_id = v_restaurant

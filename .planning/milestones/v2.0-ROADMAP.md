...
- [x] **Phase 18: Per-Tenant Data-Plane Scoping + Isolation CI** - Scope every order/customer read and write by a non-defaultable `tenant_id`, and prove cross-tenant isolation with an automated CI test [TEN-04, TEN-05] — COMPLETE (2026-06-20, code/CI verified passed; live strapi-DB apply + CMS rebuild + prod n8n import deferred)
...
- [x] **Phase 21: UI Fail-Closed Parity + Module-Key Alignment + Type Cleanup** - Default `useEntitlements.hasModule` to false on loading/error, align `App.tsx` nav keys to the seeder, and replace the `any` debt with typed entitlement DTOs so Frontend Lint goes green [ENT-01, ENT-02, TYP-01] (completed 2026-06-20)
...
Plans:
- [x] 19-01-PLAN.md [AUD-01] — ADR `docs/adr/0003-entitlement-audit-placement.md` (cross-DB: table stays in the strapi DB, raw-Knex `strapi.db.connection` writer since it is NOT a content type; locks the `ralphe:entitlement:{tenant_id}:{module_key}` cache-key contract + records O-1/O-2/O-3) + idempotent live-safe `db/migrations-strapi/2026-06-20_entitlement_audit_uuid.sql` (`tenant_id` VARCHAR→uuid + **nullable** FK — global product-module rows carry `tenant_id`=NULL; proven idempotent apply-twice on ephemeral PG; 🔴 VPS apply deferred)
- [x] 19-02-PLAN.md [AUD-01, AUD-02] — Pure `audit-hook.ts` helper (deriveAction + zod `z.string().guid()` canonical-UUID validate + raw-Knex audit insert + exact-key Redis DEL) wired through `tenant-entitlement/lifecycles.ts` (audit + invalidate; old→new captured in before* hooks, actor via `requestContext`) and `product-module/lifecycles.ts` (audit-only — O-1, maps `key`→`module_key`, `tenant_id`=NULL); fail-loud (validate-throw-pre-write, log-error+count-post-commit, no bare swallow); tsc baseline unchanged (5/5, NO new CMS TS errors); 10/10 node-tests + 4/4 SQL assertions pass on ephemeral PG+Redis
- [x] 19-03-PLAN.md [AUD-02] — Validation infra: `db/ci-fixtures/19-entitlement-audit-seed.sql` + `db/ci-assertions/19-entitlement-audit.sql` (row-per-op + invalid-uuid negative + product-module NULL-tenant path, nested `BEGIN..EXCEPTION`, no SAVEPOINT) + `__tests__/audit-hook.test.mjs` (`node --test`: SET→`invalidateCache`→GET-nil on the canonical key + the audit-row write) + `scripts/test-phase19.sh` (ephemeral PG + `redis-server`, docker DOWN) + `.github/workflows/phase-19-assertions.yml` (mirrors phase-18 + adds a `redis:7-alpine` service + **pinned Node 22** for the `node --test` type-stripping)
...
| 21. UI Fail-Closed Parity + Module-Key Alignment + Type Cleanup | 4/4 | Complete   | 2026-06-20 |
